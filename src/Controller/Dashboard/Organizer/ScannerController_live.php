...
class ScannerController extends Controller
{
    ...
    public function index(Request $request, PaginatorInterface $paginator, AppServices $services)
    {
        ...
        // Get the organizer's slug
        $organizerSlug = $user->getOrganizer()->getSlug();

        // Define search criteria
        $criteria = array(
            "role" => "scanner",
            "createdbyorganizerslug" => $organizerSlug,
            "username" => $username,
            "enabled" => $enabled
        );

        // Get users matching the criteria
        $query = $services->getUsers($criteria)->getQuery();

        // Paginate the results
        $page = $request->query->getInt('page', 1);
        $itemsPerPage = 10;
        // dd($paginator->paginate($query, $page, $itemsPerPage));
        $scanners = $paginator->paginate($query, $page, $itemsPerPage);
        // dd($enabled);
