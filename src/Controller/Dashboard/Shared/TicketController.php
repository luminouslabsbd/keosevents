...
class TicketController extends Controller
{
    ...
    public function mailServerTest(Request $request, AppServices $services, Swift_Mailer $mailer, Environment $templating, TranslatorInterface $translator, Connection $connection)
    {
        ...
        $email = (new \Swift_Message($email_subject_title))
            ...
            ->setTo($emailTo)
            ->setBody(
                $this->renderView('Dashboard/Shared/Order/confirmation-email.html.twig', ['order' => $orders]),
                'text/html'
            )
            ...
            if ($eventMail['status'] == 0) {
                ...
                if ((preg_match($pattern, $emailTo))) {
                    $email = (new \Swift_Message($email_subject_title))
                        ...
                        ->setTo($emailTo)
                        ->setBody(
                            $this->renderView('Dashboard/Shared/Order/confirmation-email.html.twig', ['order' => $order]),
                            'text/html'
                        )
